...
-- =============================================================================
-- LV Biofuel Refinery (Powered / Appliances)
-- =============================================================================

if minetest.get_modpath("appliances") and minetest.get_modpath("pipeworks") and minetest.get_modpath("technic") then

assert(biomass, "Global 'biomass' table not found.")
assert(biomass.convertible_items, "Biomass convertible_items table is missing.")

if minetest.get_modpath("hopper") then
	hopper:add_container({
		{"top",    "biofuel:refinery_lv",        "output"}, -- top    of hopper removes from output
		{"bottom", "biofuel:refinery_lv",        "input"},  -- bottom of hopper inserts to   input
		{"side",   "biofuel:refinery_lv",        "input"},
		{"top",    "biofuel:refinery_lv_active", "output"},
		{"bottom", "biofuel:refinery_lv_active", "input"},
		{"side",   "biofuel:refinery_lv_active", "input"},
	})
end

local refinery_lv = appliances.appliance:new({
    node_name_inactive = "biofuel:refinery_lv",
    node_name_active   = "biofuel:refinery_lv_active",
    --node_name_waiting  = nil
    
    node_description   = S("Biofuel Refinery"),
    node_help          = S("Process biomass into liquid fuel."),
    
    -- Using "input"/"output" to ensure compatibility with library Pipeworks defaults
    input_stack        = "input", 
    input_stack_size   = 1,
    --have_input       = true
    -- FIXME formspec doesn't reflect input_stack_size if not 1

    usage_stack        = 0, -- FIXME use_stack ?
    --use_stack          = "use_in",
    --use_stack_size     = 1, 
    have_usage         = false,
...
-- 3. Data Registration
refinery_lv:item_data_register({
    ["tube_item"]     = {},
    ["techage_item"]  = {},
    ["minecart_item"] = {},
})

-- [2026-03-06] Adjusted power demands for hyper-realistic long production cycles.
-- Since production can take up to 24 hours, demand must be minimal to prevent
-- unrealistic battery drainage for what is essentially a slow chemical process.

refinery_lv:power_data_register({
    ["no_power"] = {
        disable = {}
    },
    ["LV_power"] = {
        -- 1 EU is the minimum possible draw.
        -- Over 24 hours (86,400s), this totals 86,400 EU,
        -- which is almost one full Technic LV Battery Box (100k).
        demand = 1,
        run_speed = 1,
        disable = {"no_power"}
    },
    ["power_generators_electric_power"] = {
        -- Adjusted down from 100 to 5 to align with long-term processing.
        demand = 5,
        run_speed = 1,
        disable = {"no_power"}
    },
    ["elepower_power"] = {
        -- Elepower units are often more "expensive" per point; 1 is a trickle.
        demand = 1,
        run_speed = 1,
        disable = {"no_power"}
    },
    ["techage_electric_power"] = {
        -- Techage "TA" power; 5 is a very light load (similar to a lamp).
        demand = 5,
        run_speed = 1,
        disable = {"no_power"}
    },
...
function refinery_lv:recipe_inventory_can_put(pos, listname, index, stack, player)
    -- Only allow convertible items into the input slot
    if listname == self.input_stack then
        if is_convertible(stack:get_name()) then
            return stack:get_count()
        end
    end
    return 0
...
function refinery_lv:recipe_inventory_can_take(pos, listname, index, stack, player_name)
    ...
    -- If it's the input stack and it's currently processing, don't let pipes steal it
    if listname == self.input_stack then
        local meta = minetest.get_meta(pos)
        if meta:get_int("production_time") > 0 then
...
--    
--    if is_convertible(input_name) then
--        -- [2026-02-28] Added logging to verify recipe trigger
--        minetest.log("action", "[biofuel] Refinery processing: " .. input_name)
--        
--        -- Tonumber used to ensure production_time is a valid float/int
--        local time_setting = minetest.settings:get("fuel_production_time")
--        local p_time = tonumber(time_setting) or 10
--        
--        return {
--            inputs = 1, 
--            outputs = { bottle_output and "biofuel:bottle_fuel" or "biofuel:phial_fuel" },
...
function refinery_lv:recipe_aviable_input(inventory)
    ...
    -- [2026-03-06] Scaling throughput for realism.
    -- To justify the 24h power draw, we process a larger batch at once.
    local batch_size = 16 -- Increased from 1 to 16
    local output_yield = 4 -- Increased from 1 to 4
    
    -- Ensure the input stack has enough items for the batch
    if is_convertible(input_name) and input_stack:get_count() >= batch_size then
        
        -- [2026-02-28] Added logging to verify recipe trigger
        minetest.log("action", "[biofuel] LV Refinery processing batch of " .. batch_size .. ": " .. input_name)
        
        -- Tonumber used to ensure production_time is a valid float/int
        local time_setting = minetest.settings:get("fuel_production_time")
        local p_time = tonumber(time_setting) or 10
        
        -- Determine output item based on settings
        local out_item = bottle_output and "biofuel:bottle_fuel" or "biofuel:phial_fuel"
        
        -- [2026-02-27] Assertion to ensure output yield doesn't exceed stack limits
        assert(output_yield <= 99, "Output yield per cycle is too high for a single stack.")

        return {
            inputs = batch_size, 
            outputs = { out_item .. " " .. output_yield },
            production_time = p_time,
        }, nil
    end
    return nil, nil
...
assert(refinery_def              ~= nil)
assert(refinery_def       .tiles ~= nil)
local refinery_active_def                     = minetest.registered_nodes['biofuel:refinery_active']
assert(refinery_active_def       ~= nil)
assert(refinery_active_def.tiles ~= nil)
local refinery_lv_def                         = table.copy(refinery_def)
--refinery_lv_def.groups                        = table.copy(refinery_def.groups)
--refinery_lv_def.groups['tubedevice']          = 1
--refinery_lv_def.groups['tubedevice_receiver'] = 1
refinery_lv_def.tiles                         = nil
refinery_lv_def.on_timer                      = nil
refinery_lv_def.on_construct                  = nil
refinery_lv_def.on_rightclick                 = nil
refinery_lv_def.can_dig                       = nil
refinery_lv_def.tube                          = nil
refinery_lv_def.allow_metadata_inventory_put  = nil
refinery_lv_def.allow_metadata_inventory_move = nil
refinery_lv_def.allow_metadata_inventory_take = nil
refinery_lv_def.on_metadata_inventory_put     = nil
refinery_lv_def.on_metadata_inventory_take    = nil
local refinery_lv_inactive_def                = {
	tiles = table.copy(refinery_def       .tiles),
}
--refinery_lv_inactive_def.tiles[0]             = refinery_lv_inactive_def.tiles[0] .. '^[colorize:#00FFFF:30' -- FIXME
local refinery_lv_active_def                  = {
	tiles = table.copy(refinery_active_def.tiles),
}
--refinery_lv_active_def  .tiles[0]             = refinery_lv_active_def  .tiles[0] .. '^[colorize:#00FFFF:50' -- FIXME

refinery_lv:register_nodes(refinery_lv_def, refinery_lv_inactive_def, refinery_lv_active_def)

minetest.register_craft({
    output = "biofuel:refinery_lv",
    recipe = {
        {"default:steel_ingot",  "biofuel:refinery",       "default:steel_ingot"},
